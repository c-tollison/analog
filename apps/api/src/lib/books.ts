 ...
 * haven't seen it. Series and volume come from the user and overwrite what's
 * stored, since Open Library's series data is unreliable.
 */
export async function upsertBook(
    ...
    let item = await findBookByIsbn(isbn);
    let kind = toBookKind(item?.kind);
    ...
    if (!item) {
        ...
        item = await findBookByIsbn(isbn);
        if (!item) {
